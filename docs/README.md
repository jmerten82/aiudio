...
| [pipeline/85-phase2-agent-workbench-roadmap.md](pipeline/85-phase2-agent-workbench-roadmap.md) | **Phase 2 roadmap — Agent Control Plane & Visual Workbench.** A browser **visual graph editor** + a **grounded LLM companion** + **agent self-extension** (authoring new nodes on the fly), all driving one live engine through one typed action space, grounded in one capability manifest. Architecture, the hard problems (chiefly RT-safety of agent-authored C++ + its gate), milestones across four workstreams, a five-release shipping ladder, ADRs 0019–0024, definition of done. **✅ Phase 2 complete (R1–R4); R5 split out as Phase 2.1.** |
| [pipeline/86-r5-self-extension-plan.md](pipeline/86-r5-self-extension-plan.md) | **Phase 2.1 implementation plan — agent self-extension (D0–D3).** The detailed plan for the agent authoring new node *types* at runtime (originally R5, now its own follow-on phase): the node-plugin C ABI + a runtime node registry, the local personal package registry (ADR-0023), the spec→C++ scaffold + off-thread build, the **RT-safety pre-flight gate** (static + RTSan/alloc-hook + contract → pass/quarantine/discard, ADR-0024), and the agent authoring loop. Architecture + diagram, hard-problems/feasibility table, milestones as sub-PRs, the RT-gate + plugin-ABI designs, the security model, a headless testing strategy, and honest PoC caveats. |
| [pipeline/87-phase2-testing-plan.md](pipeline/87-phase2-testing-plan.md) | **Phase 2 testing plan.** How the workbench is tested — headless-first, contract-on-both-sides, grounding-verified. What's covered where (a per-layer table: action space, manifest, server, agent, tuning, frontend logic/contract/build), the CI matrix, gated/manual tests (live agent, live audio, browser QA), a manual QA checklist, known gaps + proposed additions (Playwright E2E, jsdom component tests), and a coverage summary. |
...
| [cookbooks/88-agent-workbench.md](cookbooks/88-agent-workbench.md) | **The agent workbench cookbook** (Phase 2) — how to *use* the workbench: build/edit a graph through the typed action space (`GraphSession`), the capability manifest + validation, the server + WebSocket protocol, editing by hand in the browser, the **grounded LLM companion** (edit by natural language), **self-tuning** (`tune_to_target` — params by gradient), and embedding the session headlessly. Runnable Python + shell + a full WS message reference. |
