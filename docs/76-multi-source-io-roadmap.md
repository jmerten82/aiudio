...
- **The rate/clock tail is merged (the stacked chain PRs 8–11, all in `main`):** PR 8 (M9.3
  ...
  only hardware-verified-only steps remaining are the *physical* triggers (a real device unplug;
  two real devices on separate physical clocks), whose logic is proven headlessly via the mock.
