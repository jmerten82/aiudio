...
if [ "$SANITIZE" = 1 ]; then
    ...
    cmake --build build-tsan -j --target test_graph_control test_graph_live_edit
    ctest --test-dir build-tsan -R 'test_graph_control|test_graph_live_edit' --output-on-failure
